...
1 + 1 + 1 + 1 + 1 + 1 = 6
    ...
    if n == 0:
        return 1  #if we need to calculate (2,2), we split it into (0,2) and (2,1)
    elif n < 0:        #(0,2) has one solution, that is 0 + 2 = 2
        return 0       #then we split (2,1) into (1,1) and (2,0), in which (2,0) has no solution
        ...
        without_m = count_partitions(n, m-1)    #One is that the partitions have m, the other one is not.
...
def partition_tree(n, m):
        ...
        if n == 0:
            ...
            left = partition_tree(n-m, m)  # No matter how left changes, the m in it doesn't change.
...
def print_parts(tree, partition=[]):
    ...
    """
    if is_leaf(tree):
        if label(tree):                   # Only the left part can return the True value.
            print(' + '.join(partition))
    else:
        left, right = branches(tree)
        m = str(label(tree))
        print_parts(left, partition + [m])   # Accumulate m using partition when it is the left part
...
def partitions(n, m):
        ...
        if n == 0:
            ...
            using_m = partitions(n-m, m)    # Only using_m can return [empty, empty]
...
def print_partitions(n, m):
    ...
    strings = apply_to_all_link(lambda s: join_link(s, " + "), lists)  # Join the lists in every first element
