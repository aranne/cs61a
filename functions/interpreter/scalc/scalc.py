...
def calc_eval(exp):
    """Evaluate a Calculator expression. It's a generic funcion for primitive expression or call expression.
    ...
    10
    """
    if type(exp) in (int, float):  # if the expression is primitive expression.
        return simplify(exp)  # simplify(8.0) => 8
    elif isinstance(exp, Pair):
        arguments = exp.second.map(calc_eval)  # map calc_eval over all the rest of expression into Pair structure.
        return simplify(calc_apply(exp.first, arguments))   # the expression.first should be an operator, and reduce arguments to a number.
    else:
...
def reduce(fn, scheme_list, start):     # This is an important function.
    ...
    if scheme_list is nil:
        return start            # The reduced result is stored in start, the default start is 0.
...
@main
def read_eval_print_loop():      # This is an interactive interpreter.
    ...
    while True:
        try:
            ...
            return                              #  this is the end of the while True loop.
