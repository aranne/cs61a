...
class Buffer(object):
    """A Buffer provides a way of accessing a sequence of tokens across lines, using index processing as to be a pointer.
    ...
    >>> buf = Buffer(iter([['(', '+'], [15], [12, ')']])) # this tokens has three results in iterator.
    >>> buf.pop()
    '('
    >>> buf.pop()
    '+'
    ...
    def __init__(self, source):
        ...
        self.source = source
        self.current_line = ()
        self.current()             # When we initiate, we call the self.current() funtion to indicate the current_line.
    ...
    def more_on_line(self):
        return self.index < len(self.current_line)   # If is True, means current is still in this line, if False, the current(index) is out of current_line,
                                                        # we should to get the next(line) into the current_line, and clear the index to be 0.
    def current(self):
        """Return the current element, or None if none exists."""
        while not self.more_on_line:  # While at the end of a line, we need to change to the next line to find the tail ')'
            self.index = 0
            try:
                self.current_line = next(self.source) # The source is tokenize_lines(InputReader('> ')), call next() on the iterator tokenize_lines will then call next() on input to get the next line.
                self.lines.append(self.current_line)
            except StopIteration:
                self.current_line = ()
                return None
        return self.current_line[self.index]     # current is the one at the index in the current_line.
...
class InputReader(object):
    ...
    def __init__(self, prompt):
        self.prompt = prompt            # The first prompt should be 'read> '

    def __iter__(self):                 # The instance of InputReader is an iterator.
        while True:
            yield input(self.prompt)   # print(self.prompt) and wait for user to input, each time we input a line, type Enter to finish input.
            self.prompt = 'next_line-->  '   # if we input in multiple lines in the interpreter, then the self.prompt will be 'next_line-->  '
