...
def tokenize_line(line):            # process one line at one time.
    ...
    return result                                        # The result is a [...].

def tokenize_lines(input):  # if there're multiple lines, the interpreter will read the first line, and couldn't find one or more ')', then it will calls input in InputReader in order to wait for user to input.
    """An iterator that returns lists of tokens, one for each line of the
    iterable input sequence."""
    return map(tokenize_line, input)  # this iterator can be represented as [[line x]], contains only one line at a time. If we go to the next line, the last line's result([...]) will be cleared.
