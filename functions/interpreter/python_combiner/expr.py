...
class Name(Expr):
    ...
    def eval(self, env):       # when we eval a Name we need to put in the environment where we want the Name to be evaluated.
        """
        >>> env = {
        ...     'a': Number(1),
...
class LambdaExpr(Expr):
    ...
    def eval(self, env):
        return LambdaFunction(self.parameters, self.body, env)  # when a lambda function is evaluated, a lambda function is created, so env will be passed to LambdaFunction as a parent environment.
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.operator.eval(env).apply([operand.eval(env) for operand in self.operands])  # self.operator.eval(env) will be evaluated in the environment directory.
                                                                                              # we should first evaluate all the operand to values before passed into call function.
    def __str__(self):
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        new_env = self.parent.copy()        # Make a copy of the parent environment.
        for parameter_pair in zip(self.parameters, arguments):  # we need to bind arguments to parameters in the new environment.
            new_env[parameter_pair[0]] = parameter_pair[1]  # Why we need the parent environment? Because if there're parent frame veriables in the lambda function body, we need to look up them.
...
class PrimitiveFunction(Value):
    ...
    def apply(self, arguments):
        for arg in arguments:
            if type(arg) != Number:                                      # the arguments should be evaluate to Number first before passed into apply function.
...
global_env = {
    'abs': PrimitiveFunction(operator.abs),                     # operator is imported.
