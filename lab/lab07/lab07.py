...
def is_bst(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if not t.is_leaf():                     # To eliminate all the False situation.
        if len(t.branches) > 2:
            return False
        elif len(t.branches) == 1:
            if (t.branches[0]).label <= t.label:
                if max_tree(t.branches[0]) > t.label:
                     return False
            if (t.branches[0]).label >= t.label:
                if min_tree(t.branches[0]) < t.label:
                    return False
        ...
        for b in t.branches:
            if not is_bst(b):
                return False
    return True
    # Official answer:             # This is much better than mine.
    "*** YOUR CODE HERE ***"
    if t.is_leaf():
        return True
    if len(t.branches) > 2:
